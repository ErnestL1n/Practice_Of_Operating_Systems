...
*/
	...
	INT32U c_value;      //c_value==executiontime
	INT32U p_value;      //p_value==period
	INT32U comp_time;   //remain time
	INT32U ddl;          //deadline
	...
	INT32U end;
		...
		/*6-29*/
			((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->RemainTime--;//当前任务又执行了一个周期，所需要的执行时间减1
		
		if (((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->RemainTime == 0) {//若当前任务执行完了,更新当前任务TCB的各项指标
			((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->Deadline = ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->Deadline + ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->Period;
			((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->comp_time = ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->executiontime;
			((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->end = OSTimeGet();
			OSTCBCur->OSTCBDly = ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->period - (((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->end - ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->start);
			

			((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->start = ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->start + ((TASK_EXTRA_DATA*)OSTCBCur->OSTCBExtPtr)->p_value;
...
{
	//6-29
#if OS_LOWEST_PRIO <= 7u                        /* See if we support up to 7 tasks                   */
    INT8U   y;
