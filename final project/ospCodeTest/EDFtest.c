...
/*  wei-ting  */
/*Decrease the remaining execution time of the current task */


/*PERIODIC_TASK_START_PRIO+7(must be revised,according to the # of PeriodTasks)*/
/*Check if the task on CPU is my periodic task*/


//MACRO OSPrioCur=>目前占用CPU之工作的優先權
//Check if priority >=20 && priority<=20+7(tasks)
if((OSPrioCur>=PERIODIC_TASK_START_PRIO)&&(OSPrioCur<=PERIODIC_TASK_START_PRIO+7))
{
  /*Then the TCB has the extra object to record the Info.*/
